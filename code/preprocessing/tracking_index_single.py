...
GHI_loc=np.array(GHI_loc)[:5];

lead_minutes=[1,3,5,10,15,30,45,60]; 
lead_times=[lt/INTERVAL for lt in lead_minutes]

days=['201809{:02d}15'.format(iday) for iday in range(1,11)]
...
for day in days:
    ...
    for f in flist:
        with open(f,'rb') as input:
            img=pickle.load(input); img.cm[:-1000,:-1000]=-1
            ny,nx = img.cm.shape
            ...
            for iGHI in range(len(GHI_loc)):
                ...
                if img.cm[slc].size>=1:
                    for ilt, lead_time in enumerate(lead_times):
                        img.v[0][0]=1/img.pixel_size if img.v[0][0]==0 else img.v[0][0];
                        img.v[0][1]=0.5/img.pixel_size if img.v[0][1]==0 else img.v[0][1];
...
plt.legend(); 
plt.savefig("ti_single.png");#plt.show();
