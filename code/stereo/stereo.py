...
params = {'HD815_1':[2821.0000,1442.8231,1421.0000,0.1700,-0.0135,-2.4368,0.3465,-0.0026,-0.0038],\
          'HD815_2':[2821.0000,1424,1449,0.0310,-0.0114,-0.9816,0.3462,-0.0038,-0.0030 ],\
          'HD490':[2843.0000,1472.9511,1482.6685,0.1616,0.0210,-0.5859,0.3465,-0.0043,-0.0030], \
          'HD17':[2817.249,1478.902,1462.346,-0.099,0.012,0.867,2]}
...
#     for f in sorted(glob.glob(inpath+camera+'/'+camera+'*201803082040*jpg')):  ###1500
#     for f in sorted(glob.glob(inpath+camera+'/'+camera+'*20180303171034*jpg')):  ###1500
    for f in sorted(glob.glob(inpath+camera+'/'+camera+'*20180303154034*jpg')):  ###8200
#     for f in sorted(glob.glob(inpath+camera+'/'+camera+'*20180228144029*jpg')):####5100 m 
#     for f in sorted(glob.glob(inpath+camera+'/'+camera+'*20180228124029*jpg')):####7500 m            
#     for f in sorted(glob.glob(inpath+camera+'/'+camera+'*20180226205228*jpg')):  ###5000 m
#         print(f)
...
#         ###detrend the data
#         mk=(im0[:,:,0]>5) & (cos_g>0)
#         slope, intercept, *tmp = stats.linregress(cos_g[mk], im0[mk,0])
#         im0[:,:,0]-=(slope*cos_g+intercept)
        im0_m=st.rolling_mean2(im0[:,:,0],100,fill=np.nan)
        im0[:,:,0]-=im0_m        
        sun_line=np.abs(x0*np.cos(saz+params[camera][3] )-y0*np.sin(saz+params[camera][3] ))<40
        std=np.nanstd(im0[sun_line,0]); 
        im0[sun_line & (np.abs(im0[:,:,0])>2*std),:]=np.nan
